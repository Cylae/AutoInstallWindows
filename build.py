...
def update_autounattend(ssid=None, password=None):
    ...
    if not xml_path.exists():
        logging.error(f"Error: {xml_path} not found.")
        return

    content = xml_path.read_text(encoding='utf-8')

    # --- Script Updates ---
    # Iterate over all .ps1 files in scripts directory
    for script_file in scripts_dir.rglob('*.ps1'):
        ...
        encoded_content = html.escape(script_content, quote=False)
        ...
        if re.search(pattern, content, re.DOTALL):
            ...
            logging.warning(f"Warning: File path {target_path_str} not found in XML. Skipping.")

    # Remove empty XML passes
    passes = ['offlineServicing', 'generalize', 'auditSystem', 'auditUser']
    for p in passes:
        # Match passes that contain only whitespace between opening and closing tags
        # and capture any leading whitespace on the line to avoid orphaned indentation
        pattern = r'^[ \t]*<settings pass="' + p + r'">\s*</settings>\r?\n?'
    ...
    if ssid and password:
        ...
        safe_ssid = html.escape(ssid, quote=False).replace('"', "&quot;")
        safe_password = html.escape(password, quote=False).replace('"', "&quot;")
