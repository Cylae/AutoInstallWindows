...
def update_autounattend(ssid=None, password=None):
    xml_file = Path('autounattend.xml')
    scripts_path = Path('scripts')

    if not xml_file.exists():
        logging.error(f"Error: {xml_file} not found.")
        return
    ...
    for path in scripts_path.rglob('*.ps1'):
        ...
        if re.search(pattern, content, re.DOTALL):
            ...
            if re.search(extensions_close_pattern, content):
                ...
                logging.error("Error: </Extensions> tag not found in autounattend.xml. Cannot append new script.")
