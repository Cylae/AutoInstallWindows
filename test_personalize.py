from personalize import xml_encode, get_current_value, update_value
from personalize import apply_personalizations
...
def test_get_current_value():
    ...
    pattern = r'<LocalAccount[^>]*>\s*<Name>([^<]*)</Name>'
    assert get_current_value(content, pattern) == 'Admin'
    assert get_current_value(content, r'<Missing>([^<]*)</Missing>') == ''
    assert get_current_value(
        content, r'<Missing>([^<]*)</Missing>', default='Def') == 'Def'
