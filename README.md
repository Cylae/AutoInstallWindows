...
*   **Lint & Test**: Run `flake8` to check formatting (79-character limit), `autopep8 --in-place --aggressive --aggressive <file>` to format, and `pytest` to run tests.
...
*   **Lint & Test**: Exécutez `flake8` pour vérifier le formatage (limite de 79 caractères), `autopep8 --in-place --aggressive --aggressive <fichier>` pour formater, et `pytest` pour exécuter les tests.
