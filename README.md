...
*   **Testing & Linting**: Run `pytest` to execute unit tests and `flake8` to enforce a 79-character maximum line length. You can also use `autopep8` for formatting Python code.
...
*   **Tests & Linting**: Exécutez `pytest` pour lancer les tests unitaires et `flake8` pour imposer une longueur maximale de 79 caractères par ligne. Vous pouvez également utiliser `autopep8` pour formater le code Python.
