...
*   **Build**: Run `python build.py` to regenerate `autounattend.xml` with your changes. (Now uses `pathlib` and `logging` for robust builds). Tests can be run with `pytest` and code linted with `flake8`.
...
*   **Construire**: Exécutez `python build.py` pour régénérer `autounattend.xml` avec vos changements. (Utilise désormais `pathlib` et `logging` pour des builds robustes). Les tests peuvent être exécutés avec `pytest` et le code vérifié avec `flake8`.
