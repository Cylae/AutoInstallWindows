...
*   **🛡️ Privacy Hardened**: Disables Telemetry, **Recall** (AI Screenshot), Copilot, Bing Search, Cortana, Advertising ID, **Search Highlights**, **Widgets**, **File Explorer Ads**, **Typing Insights**, **Tailored Experiences**, and **Tips/Suggestions**. **Restores Classic Context Menu** (Windows 11) and shows file extensions by default.
...
*   **📜 Log Persistence**: Setup logs are preserved in `C:\Windows\Panther\Autounattend_Log.txt` with millisecond precision for troubleshooting.
*   **🚀 System Optimization**:
    ...
    *   **Performance**: Disables App Launch Tracking (`DisableMFUTracking` & `NoInstrumentation`).
...
*   **🛡️ Confidentialité Renforcée**: Désactive la télémétrie, **Recall** (Capture d'écran IA), Copilot, la recherche Bing, Cortana, l'ID publicitaire, **Widgets**, **Publicités Explorateur**, **Aperçus de saisie (Typing Insights)**, **Expériences personnalisées**, et **Astuces**. **Restaure le menu contextuel classique** (Windows 11) et affiche les extensions de fichiers.
...
*   **📜 Journaux**: Les journaux d'installation sont conservés dans `C:\Windows\Panther\Autounattend_Log.txt` avec une précision à la milliseconde pour le dépannage.
*   **🚀 Optimisation Système**:
    ...
    *   **Performance**: Désactive le suivi du lancement des applications (`DisableMFUTracking` & `NoInstrumentation`).
