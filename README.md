...
*   **Linting & Testing**: You can use `flake8` for Python code analysis and `pwsh -c 'Invoke-ScriptAnalyzer -Path ./scripts -Recurse'` for PowerShell scripts. Run `pytest` to execute unit tests.
...
*   **Linting & Testing**: Vous pouvez utiliser `flake8` pour l'analyse du code Python et `pwsh -c 'Invoke-ScriptAnalyzer -Path ./scripts -Recurse'` pour les scripts PowerShell. Lancez `pytest` pour exécuter les tests unitaires.
