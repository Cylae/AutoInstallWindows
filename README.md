...
*   **Testing**: The project includes unit tests for `personalize.py`. Run tests using `pytest test_personalize.py`. Linting and formatting are enforced with `flake8` and `autopep8`.
...
*   **Test** : Le projet inclut des tests unitaires pour `personalize.py`. Exécutez les tests à l'aide de `pytest test_personalize.py`. Le linting et le formatage sont appliqués avec `flake8` et `autopep8`.
