...
*   **Formatting**: Python scripts are formatted with `autopep8` and linted with `flake8`. Fix scripts have been removed.
...
*   **Formatage**: Les scripts Python sont formatés avec `autopep8` et vérifiés avec `flake8`. Les scripts de correction ont été supprimés.
