...
*   **🛡️ Enhanced Robustness**:
    ...
    *   **Robust Registry Management**: Uses a custom PowerShell helper (`Set-RegistryKey`) instead of `reg.exe` to guarantee correct and robust application of all registry tweaks.
...
*   **🛡️ Robustesse Améliorée**:
    ...
    *   **Gestion Robuste du Registre**: Utilise un script PowerShell personnalisé (`Set-RegistryKey`) au lieu de `reg.exe` pour garantir l'application correcte et robuste de tous les ajustements du registre.
