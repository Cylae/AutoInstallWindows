...
*   **Testing & Linting**: Use `pytest` for executing unit tests. Formatting and linting are enforced using `flake8`, `autopep8`, and `autoflake`.
...
*   **Tests & Linting**: Utilisez `pytest` pour exécuter les tests unitaires. Le formatage et le linting sont effectués avec `flake8`, `autopep8` et `autoflake`.
