...
*   **🛡️ Enhanced Robustness**:
    ...
    *   **Advanced Error Handling**: Improved logging and registry handling for fail-safe execution. Uses `Set-RegistryKey` helper for robust registry operations instead of `reg.exe`.
...
*   **🛡️ Robustesse Améliorée**:
    ...
    *   **Gestion d'Erreurs Avancée**: Journaux améliorés et gestion robuste du registre pour une exécution sans faille. Utilise l'assistant `Set-RegistryKey` pour des opérations de registre robustes au lieu de `reg.exe`.
