...
*   **🛡️ Privacy Hardened**: Disables Telemetry, Copilot, Bing Search, Cortana, Advertising ID, Search Highlights, **Widgets**. **Restores Classic Context Menu** (Windows 11) and shows file extensions by default. **Enforced Edge Policies** to disable first-run experience and background services.
*   **🌑 Dark Mode**: Enables Dark Mode for System and Apps by default.
*   **🧹 Deep Debloating**: Aggressively removes bloatware using **intelligent matching** (Display Name & Package Name) to ensure complete removal of Candy Crush, Clipchamp, **Widgets**, **Cortana**, **Xbox/Gaming Overlays**, **YourPhone**, etc.
...
*   **🔄 Transparent Auto-Updates**:
    *   **Daily Updates**: Registers a hidden scheduled task to run `winget upgrade --all` daily (includes `source update`). Includes **smart availability check** to skip registration if Winget is not installed.
...
*   **🛡️ Confidentialité Renforcée**: Désactive la télémétrie, Copilot, la recherche Bing, Cortana, l'ID publicitaire, **Widgets**. **Restaure le menu contextuel classique** (Windows 11) et affiche les extensions de fichiers. **Politiques Edge appliquées** pour désactiver l'expérience de premier lancement et les services en arrière-plan.
*   **🌑 Mode Sombre**: Active le mode sombre pour le système et les applications par défaut.
*   **🧹 Nettoyage en Profondeur**: Supprime agressivement les bloatwares avec une **correspondance intelligente** (Nom d'affichage & Nom de paquet) pour garantir la suppression complète de Candy Crush, Clipchamp, **Widgets**, **Cortana**, **Xbox/Jeux**, **YourPhone**, etc.
...
*   **🔄 Mises à Jour Automatiques Transparentes**:
    *   **Quotidien**: Enregistre une tâche planifiée masquée pour exécuter `winget upgrade --all` chaque jour. Inclut une **vérification intelligente** pour ignorer l'enregistrement si Winget n'est pas installé.
