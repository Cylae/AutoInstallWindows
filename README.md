...
*   **Testing & Linting**: Use `pytest` for executing unit tests, `flake8` for linting (enforcing a 79-character maximum line length), and `autopep8` for formatting Python code. Run `pytest` and `flake8 .` before committing changes.
...
*   **Tests et Linting**: Nous utilisons `pytest` pour l'exécution des tests unitaires, `flake8` pour le linting (imposant une longueur maximale de 79 caractères par ligne), et `autopep8` pour le formatage du code Python. Exécutez `pytest` et `flake8 .` avant de valider vos modifications.
